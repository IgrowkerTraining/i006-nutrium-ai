...
async def analyze_nutritionist_match(
    request: MatchAnalysisRequest, ai_service: AIService = Depends(get_ai_service)
):
    """
    ...
    **Example Use Case:**
    ...
    try:
        logger.info(
            f"Received match analysis request for {len(request.nutritionists)} nutritionists"
        )
        ...
        logger.info(
            f"Match analysis successful. Returned {len(response.matches)} matches"
        )
        return response
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to analyze matches: {str(e)}"
        )
