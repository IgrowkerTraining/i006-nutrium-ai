...
class MatchingService:
    ...
    async def analyze_match(self, request: MatchAnalysisRequest) -> MatchAnalysisResponse:
        """
        ...
        """
        logger.info(f"Starting match analysis for {len(request.nutritionists)} nutritionists")
        
        # Build the AI prompt
        prompt = self._build_matching_prompt(
            request.patient_profile,
            request.nutritionists
        )
        ...
        try:
            ...
            matches = self._parse_ai_response(
                ai_response,
                request.nutritionists
            )
            ...
            response = MatchAnalysisResponse(
                ...
                matches=top_matches,
                analysis_summary=self._generate_summary(request.patient_profile, top_matches),
                model_used=request.ai_model
            )
            
            logger.info(f"Match analysis completed. Found {len(top_matches)} top matches")
            return response
...
"""
    ...
    async def _get_ai_analysis(self, prompt: str, model: str) -> str:
        ...
        chat_request = ChatRequest(
            model=model,
            messages=[
                ChatMessage(role="system", content="Eres un experto en nutrición que analiza compatibilidad entre pacientes y nutricionistas. Respondes solo con JSON válido."),
        ...
        try:
            ...
            for match_data in matches_data:
                ...
                match = NutritionistMatch(
                    ...
                    bio_excerpt=nutritionist.bio[:150] + "..." if nutritionist.bio and len(nutritionist.bio) > 150 else nutritionist.bio
                )
    ...
    def _generate_summary(
        self,
        patient: PatientProfileRequest,
        matches: List[NutritionistMatch]
    ) -> str:
