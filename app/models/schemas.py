...
class PatientProfileRequest(BaseModel):
    ...
    preferred_gender: Optional[str] = Field(default=None, description="Preferred nutritionist gender")
    budget_range: Optional[str] = Field(default=None, description="Budget range for consultations")
...
class NutritionistProfileData(BaseModel):
    ...
    accepts_new_patients: bool = Field(default=True, description="Accepting new patients")
...
class MatchReason(BaseModel):
    ...
    weight: float = Field(..., ge=0.0, le=1.0, description="Impact weight on final score")
...
class NutritionistMatch(BaseModel):
    ...
    compatibility_score: float = Field(..., ge=0.0, le=100.0, description="Compatibility score")
...
class MatchAnalysisRequest(BaseModel):
    ...
    patient_profile: PatientProfileRequest = Field(..., description="Patient profile data")
    nutritionists: List[NutritionistProfileData] = Field(..., description="List of nutritionists to analyze", min_items=1)
    top_n: int = Field(default=5, ge=1, le=20, description="Number of top matches to return")
    ai_model: str = Field(default="openai/gpt-4", description="AI model to use for analysis")
...
class MatchAnalysisResponse(BaseModel):
    ...
    matches: List[NutritionistMatch] = Field(..., description="Top matched nutritionists")
